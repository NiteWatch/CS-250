...
int TestPush()
{
    ...
    vector<string> values = GetTestItems( ARBITRARY_SIZE );

    /*
    Test 1
    Make sure items will be added appropriately.
    */
...
int TestInsert()
{
    ...
    */
	LinkedList<string> arr2, solutionArray;
	arr2.Push( "A" );
	arr2.Push( "B" );
	arr2.Push( "C" );
	arr2.Push( "D" );
    ...
    totalTests++;
    bool isSame = ( arr2 == solutionArray );
    isSame = ( arr2 == solutionArray );
    if ( !isSame )
    {
        ...
        ReportFailure( "TestInsert() Test " + StringUtil::ToString( totalTests ), message );
    }
	else { totalSuccess++; }
...
int TestExtend()
{
	...
	*/
	LinkedList<string> arr1, arr2, solutionArray;
	...
	totalTests++;
	if ((arr1 == solutionArray) == false) { ReportFailure("TestExtend() Test " + StringUtil::ToString(totalTests), "Tried to extend an array. Resulting solution wasn't correct. \n Expected: " + solution + ", received: " + actual); }
	else { totalSuccess++; }
...
int TestPop()
{
    ...
    */
    ...
    totalTests++;
    if ( arr2.Size() != ARBITRARY_SIZE ) { ReportFailure( "TestPop() Test " + StringUtil::ToString( totalTests ), "Array's size after pushing ARBITRARY_SIZE items is not ARBITRARY_SIZE." ); }
    else { totalSuccess++; }
...
int TestRemove()
{
    ...
    */
    ...
    totalTests++;
    if ( arr1.Remove( ARBITRARY_SIZE ) != false ) { ReportFailure( "TestRemove()", "Tried to remove an item at position ARBITRARY_SIZE. Should have returned false, but returned true." ); }
    else { totalSuccess++; }
...
int TestGet()
{
    ...
    */
    ...
    totalTests++;
    if ( arr1.Get( ARBITRARY_SIZE ) != "" ) { ReportFailure( "TestGet() Test " + StringUtil::ToString( totalTests ), "Created array. Tried to access index ARBITRARY_SIZE. Result wasn't the default value of empty string." ); }
    else { totalSuccess++; }
...
int TestSubscriptOperator()
{
    ...
    */
	...
	totalTests++;
	if ( arr1.Get( ARBITRARY_SIZE - 2 ) != "" ) { ReportFailure( "TestSubscriptOperator() Test " + StringUtil::ToString( totalTests ), "Tried to use Get() on empty array, and it didn't return an empty string!" ); }
    else { totalSuccess++; }

	totalTests++;
	if ( arr1.Get( ARBITRARY_SIZE + 2 ) != "" ) { ReportFailure( "TestSubscriptOperator() Test " + StringUtil::ToString( totalTests ), "Tried to use Get() for out-of-bounds index, and it didn't return an empty string!" ); }
    else { totalSuccess++; }
